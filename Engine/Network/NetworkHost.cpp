...
NetworkHost::NetworkHost(HostType type, std::unique_ptr<ISocket> socket, uint32_t max_peers, uint8_t max_channels)
...
NetworkHost::NetworkHost(HostType type, std::unique_ptr<ISocket> socket, const NetworkAddress &address, uint32_t max_peers, uint8_t max_channels)
    : NetworkHost(type, std::move(socket), max_peers, max_channels)
{
    m_address = address;
...
NetworkHost::NetworkHost(HostType type, uint32_t max_peers, uint8_t max_channels)
    : NetworkHost(type, std::make_unique<Socket>(AddressFamily::IPv4), max_peers, max_channels)
{}

NetworkHost::NetworkHost(HostType type, const NetworkAddress &address, uint32_t max_peers, uint8_t max_channels)
    : NetworkHost(type, std::make_unique<Socket>(AddressFamily::IPv4), address, max_peers, max_channels)
{}
...
void NetworkHost::SendPacket(PeerId peer_id, ChannelId channel, std::unique_ptr<Packet> packet) {
    ...
    peer.outgoing_packet_queue.emplace(std::move(packet));
...
void NetworkHost::ReceivePackets() {
    ...
    while (true) {
        ...
        if (packet->Type() == PacketType::DisconnectRequest) {
            m_outgoing_packet_queue.emplace(from, std::make_unique<DisconnectResponsePacket>());
            ...
            if (peer_id && m_peers[*peer_id].status == PeerStatus::Connected) {
                ...
                channel.incoming_command_queue.emplace(std::move(command));
            }
            continue;
        }
        ...
        if (packet->Type() == PacketType::ConnectionRequest) {
            ...
            if (!peer_id) {
                ...
                if (!peer_id) {
                    ...
                    m_outgoing_packet_queue.emplace(from, std::move(response));
                    continue;
                } else {
...
std::optional<PeerId> NetworkHost::FindSlot(const NetworkAddress &address) {
    auto it = std::ranges::find_if(m_peers, [address](const NetworkPeer &peer) {
...
std::optional<PeerId> NetworkHost::FindEmptySlot() {
    auto it = std::ranges::find_if(m_peers, [](const NetworkPeer &peer) {
...
std::vector<std::unique_ptr<Packet>> NetworkHost::SplitIntoFragments(const Packet &packet) {
    ...
    for (; remaining_packet_bytes > 0; remaining_packet_bytes -= MAX_PACKET_FRAGMENT_SIZE) {
        auto data = buffer.ReadBytes<MAX_PACKET_FRAGMENT_SIZE>(std::min(static_cast<uint16_t>(remaining_packet_bytes), MAX_PACKET_FRAGMENT_SIZE));
...
std::vector<std::unique_ptr<Packet>> NetworkHost::SplitIntoSlices(uint16_t chunk_id, const Packet &packet) {
    ...
    const int header_footer_size = PACKET_DATA_BEGIN + PACKET_DATA_END;
    int remaining_packet_bytes = static_cast<int>(buffer.GetSize()) - header_footer_size;
    auto slice_count = static_cast<uint8_t>(std::floor(remaining_packet_bytes / MAX_PACKET_SLICE_SIZE));
    ...
    for (; remaining_packet_bytes > 0; remaining_packet_bytes -= MAX_PACKET_SLICE_SIZE) {
        auto data = buffer.ReadBytes<MAX_PACKET_SLICE_SIZE>(std::min(static_cast<uint16_t>(remaining_packet_bytes), MAX_PACKET_SLICE_SIZE));
