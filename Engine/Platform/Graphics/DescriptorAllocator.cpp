...
VkDescriptorSet DescriptorAllocator::AllocateDescriptorSet(VkDescriptorSetLayout layout) {
    ...
    VkResult result = vkAllocateDescriptorSets(m_device.Device(), &allocate_info, &descriptor_set);
    if (result == VK_ERROR_OUT_OF_POOL_MEMORY || result == VK_ERROR_FRAGMENTED_POOL) {
        m_full_pools.push_back(pool);
        pool = GetDesriptorPool();
        allocate_info.descriptorPool = pool;
        VK_CHECK(vkAllocateDescriptorSets(m_device.Device(), &allocate_info, &descriptor_set));
    }
    m_ready_pools.push_back(pool);
...
VkDescriptorPool DescriptorAllocator::CreateDescriptorPool() {
    ...
    for (uint32_t i = 0; i < m_ratios.size(); ++i) {
        ...
        sizes[i].descriptorCount = static_cast<uint32_t>(std::ceil(static_cast<float>(m_sets_per_pool) * m_ratios[i].ratio));
    }
    ...
    VK_CHECK(vkCreateDescriptorPool(m_device.Device(), &descriptor_pool_create_info, nullptr, &pool));
    return pool;
}
