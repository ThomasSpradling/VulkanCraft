...
uint32_t Socket::SendTo(const NetworkAddress &to, const NetworkBuffer &data) {
    ...
    int data_sent = sendto(m_handle, data.GetData(), static_cast<int>(data.GetSize()), 0, to.Data(), to.m_size);
...
SocketError Socket::ReceiveFrom(NetworkAddress &from, NetworkBuffer &buffer) {
    ...
    int data_received = recvfrom(m_handle, buffer.GetData(), static_cast<int>(buffer.GetSize()), 0, reinterpret_cast<sockaddr*>(&addr), &size);
...
#ifdef PLATFORM_WINDOWS
    auto non_blocking = static_cast<DWORD>(value);
